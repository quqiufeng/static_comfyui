...
void* torch_std_zeros(int64_t* shape, int ndim, int dtype) {
    try {
        return wrap(torch::zeros(to_shape(shape, ndim), make_options(dtype)));
    } catch (const std::exception& e) {
...
void* torch_std_full(int64_t* shape, int ndim, double value, int dtype) {
    try {
        return wrap(torch::full(to_shape(shape, ndim), value, make_options(dtype)));
    } catch (const std::exception& e) {
...
void* torch_std_randn(int64_t* shape, int ndim, int dtype) {
    try {
        return wrap(torch::randn(to_shape(shape, ndim), make_options(dtype)));
    } catch (const std::exception& e) {
...
void* torch_std_randint(int64_t low, int64_t high, int64_t* shape, int ndim, int dtype) {
	try {
		return wrap(torch::randint(low, high, to_shape(shape, ndim), make_options(dtype)));
	} catch (const std::exception& e) {
...
void torch_std_manual_seed(int64_t seed) {
    try { torch::manual_seed(seed); } catch (...) {}
}
...
static at::Tensor resblock_forward(
    ...
    h = at::silu(h);
    h = at::conv2d(h, w.conv1_w, w.conv1_b, {1,1}, {1,1});
    ...
    h = at::silu(h);
    h = at::conv2d(h, w.conv2_w, w.conv2_b, {1,1}, {1,1});

    // Skip connection
    if (w.skip_w.defined()) {
        auto skip = at::conv2d(x, w.skip_w, w.skip_b, {1,1});
        h = h + skip;
    } else {
        h = h + x;
    }
...
void* torch_std_sd_unet_forward(
    ...
    try {
        ...
        // Input conv
        auto h = at::conv2d(x, w_lora(4), w_lora(5), {1,1}, {1,1});  // input_blocks.0.0
        ...
        h = at::conv2d(h, w_lora(28), w_lora(29), {2,2}, {1,1});
        skips.push_back(h);
        ...
        h = at::conv2d(h, w_lora(78), w_lora(79), {2,2}, {1,1});
        skips.push_back(h);
        ...
        h = at::conv2d(h, w_lora(128), w_lora(129), {2,2}, {1,1});
        skips.push_back(h);
        ...
        h = at::silu(h);
        h = at::conv2d(h, w_lora(416), w_lora(417), {1,1}, {1,1});
...
void torch_std_save_image(void* tensor, const char* path, int as_pgm) {
    try {
        ...
        // Handle various shapes: (C,H,W), (H,W,C), (H,W)
        at::Tensor img;
        if (cpu_t.dim() == 3) {
        ...
        // Clamp to [0,255]
        img = at::clamp(img, 0, 255).to(torch::kUInt8);
...
void* torch_std_sample_euler(void* noise_pred, void* x_t, void* sigma_t, void* sigma_prev) {
    try {
        ...
        auto& xt = unwrap(x_t);
        auto& sig_t = unwrap(sigma_t);      // sigma at t
        auto& sig_prev = unwrap(sigma_prev); // sigma at t-1

        // d = (x_t - eps) / sigma_t
        auto d = (xt - eps) / sig_t;

        // x_prev = x_t + (sigma_prev - sigma_t) * d
...
void* torch_std_sampler_sigmas(int steps, double sigma_min, double sigma_max,
    ...
    try {
        ...
        if (sched == "karras") {
            ...
            auto step = (max_pow - min_pow) / (steps - 1);
...
void* torch_std_controlnet_forward(
    ...
    try {
        ...
        // Input hint conv
        auto hint_conv = at::conv2d(h, w(4), w(5), {1,1}, {1,1});

        // Input blocks (same as UNet encoder but with hint added)
        auto h_in = at::conv2d(x, w(6), w(7), {1,1}, {1,1}) + hint_conv;
...
void* torch_std_vae_decode(void* vae_module, void* latent) {
    try {
        ...
        // lat: (B, 4, H, W) float
        auto decoded = vae->forward({lat / 0.18215f}).toTensor();
        return wrap(decoded);
    } catch (const std::exception& e) {
...
#ifdef WITH_LIBPNG
    // Full PNG save with libpng
    std::cerr << "PNG save requires -DWITH_LIBPNG" << std::endl;
#else
...
static at::Tensor sdxl_gn(const at::Tensor& x,
                           const std::unordered_map<std::string, at::Tensor>& d,
...
void* torch_std_sdxl_unet_forward(
    ...
    try {
        ...
        auto& inp = unwrap(inp_ptr);
        auto& ts = unwrap(timestep_ptr);
        auto& txt = unwrap(text_emb_ptr);
        auto& pool = unwrap(pooled_emb_ptr);
        auto dev = inp.device();
        auto dt = inp.dtype();

        // Timestep embed
        auto te = timestep_embedding(ts, 256);
        te = at::silu(sdxl_linear(te, d, "time_embed.0"));
        te = sdxl_linear(te, d, "time_embed.2");

        // Added conditioning embed
        auto mk = [&](double v) { return at::full({(int)ts.size(0),1}, v,
                                    at::TensorOptions().dtype(dt).device(dev)); };
        auto ae = at::cat({pool, mk(os_h), mk(os_w), mk(crop_t), mk(crop_l), mk(ts_h), mk(ts_w)}, 1);
        ae = at::silu(sdxl_linear(ae, d, "add_embed.0"));
        ae = sdxl_linear(ae, d, "add_embed.2");
        te = te + ae;

        // Conv in
        auto h = sdxl_conv2d(inp, d, "conv_in", 1, 1);

        // Encoder
        std::vector<at::Tensor> sk;
        sk.push_back(h);
        h = sdxl_down0(h, te, txt, d, "down_blocks.0", 2, 5, sk);
        h = sdxl_conv2d(h, d, "down_blocks.0.downsamplers.0.conv", 2, 1);
        sk.push_back(h);
        h = sdxl_down0(h, te, txt, d, "down_blocks.1", 2, 10, sk);
        h = sdxl_conv2d(h, d, "down_blocks.1.downsamplers.0.conv", 2, 1);
        sk.push_back(h);
        h = sdxl_down0(h, te, txt, d, "down_blocks.2", 2, 20, sk);

        // Mid
        h = sdxl_resblock(h, te, d, "mid_block.resnets.0");
        h = sdxl_attn_layer(h, txt, d, "mid_block.attentions.0", 20);
        h = sdxl_resblock(h, te, d, "mid_block.resnets.1");

        // Decoder
        auto up = sdxl_up0(h, te, txt, d, "up_blocks.0", 2, 20, sk, 5);
        h = up.back();
        h = at::upsample_bilinear2d(h, {}, true, 2.0, 2.0);
        h = sdxl_conv2d(h, d, "up_blocks.0.upsamplers.0.conv", 1, 1);

        up = sdxl_up0(h, te, txt, d, "up_blocks.1", 2, 10, sk, 2);
        h = up.back();
        h = at::upsample_bilinear2d(h, {}, true, 2.0, 2.0);
        h = sdxl_conv2d(h, d, "up_blocks.1.upsamplers.0.conv", 1, 1);

        up = sdxl_up0(h, te, txt, d, "up_blocks.2", 2, 5, sk, 0);
        h = up.back();

        // Out
        h = at::silu(sdxl_gn(h, d, "conv_norm_out"));
        h = sdxl_conv2d(h, d, "conv_out", 1, 0);
        return wrap(h);
    } catch (const std::exception& e) {
...
void* torch_std_sdxl_dual_clip(void* clip1, void* clip2, void* token_ids) {
    try {
        ...
        at::Tensor emb1;
        if (out1.isTuple()) emb1 = out1.toTuple()->elements()[0].toTensor();
        else emb1 = out1.toTensor();
        // (1,77,768)
        ...
        if (out2.isTuple()) {
            ...
            emb2 = tup[0].toTensor();
            // Pooled output might be in tup[1] or tup[0].squeeze()
        ...
        if (!pooled.defined()) {
            // Fallback: use [0] token embedding
            pooled = emb2.select(1, 0); // (1,1280)
        }

        // Concatenate: (1,77,2048)
        auto text_emb = at::cat({emb1, emb2}, 2);

        // Return dict as pair of tensors
        // We use a simple vector-based approach: just return the concatenated emb
        // For pooled, we'll have separate function
        return wrap(at::cat({text_emb.flatten(), pooled.flatten()}).reshape({-1}));
        // ^ This is hacky. Better: return via two separate functions.
        // Actually the UNet function takes both separately.
        // The user will call clip_forward directly. This function just shows pattern.
    } catch (const std::exception& e) {
...
void* torch_std_fm_sigmas(int steps, double sigma_min, double sigma_max) {
    try {
        ...
        // Reverse so sigmas[0] = 0 (clean), sigmas[N] = 1 (noise)
        sigmas = 1.0 - sigmas;
    ...
    int height, int width, int patch_size)
{
    try {
        ...
        // Patch embed
        auto h = at::conv2d(x, w(1), w(2), {patch_size, patch_size}, {0, 0});  // (1, D, H/p, W/p)
