...
# =============================================================================
#
# 【与 backup.sh（人像甜点）的差异 —— 风景/风格图两处水土不服的修正】
#   1) 默认关闭质量前缀 NO_QUALITY_PREFIX=1 → 透传 --no-quality-prefix。
#      img_hires 内置前缀是写实人像取向（photorealistic, professional portrait,
#      medium shot, realistic skin texture, soft lighting...），会把油画/动漫/
#      山水/水彩风强行拉向"照片+人像"，风格冲突且 centered/medium shot 框构图。
#   2) CFG 3.0 → 4.0。人像词短 3.0 够贴；场景词长而密（结构/笔触/光照多约束），
#      3.0 约束不足易糊，场景甜点 3.5~4.5（CFG_SCALE 可调）。
#
# 【沿用 backup.sh 的 E1xMIN 采样甜点（14 组扫描结论, 对场景同样适用的骨架）】
...
#
# 【示例】
#   风景:  ./backup_scene.sh "shan shui landscape..." ~/out.png 2560 1440
#   要前缀: NO_QUALITY_PREFIX=0 ./backup_scene.sh "..." ~/out.png 2560 1440
#   LoRA:   ./backup_scene.sh "prompt" ~/out.png 2560 1440 --lora style.safetensors:0.8
# =============================================================================
set -euo pipefail
...
# 场景图默认关质量前缀（写实人像前缀污染风格词）; NO_QUALITY_PREFIX=0 恢复 img_hires 默认前缀
