#!/bin/bash
# =============================================================================
# img_hires.sh - SDXL HiRes Fix + VAE tiling wrapper for cpp/sd compatibility layer
# =============================================================================
# Usage: ./img_hires.sh "prompt" ~/output.png 2560 1440
#
# Environment variables:
#   SD_CLI            - path to img_hires binary (default: ./build/img_hires)
#   MODEL_DIR         - model directory (default: /data/models/image)
#   DIFFUSION_MODEL   - UNet/diffusion model (default: sd_xl_base_1.0.safetensors)
#   CLIP_L_MODEL      - CLIP-L model (default: clip_l.safetensors)
#   CLIP_G_MODEL      - CLIP-G model (default: clip_g.safetensors)
...
#   CFG_SCALE         - CFG scale (default: 3.2)
...
for model in "$DIFFUSION_MODEL" "$CLIP_L_MODEL" "$CLIP_G_MODEL" "$VAE_MODEL"; do
...
# Convert VAE_TILE_SIZE "128x128" -> single integer "128" (our binary expects one int)
...
NEGATIVE_PROMPT="${NEGATIVE_PROMPT:-blurry, low quality, worst quality, jpeg artifacts, noise, grain, soft focus, out of focus, hazy, unclear, bad anatomy, deformed}"
