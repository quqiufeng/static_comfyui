...
struct ST{
    uint8_t*map=nullptr;size_t fsize=0;uint64_t hs=0;uint8_t*data(){return map+8+hs;}
...
bool ST::load(const char*p){
    int fd=::open(p,O_RDONLY);if(fd<0)return false;fsize=(size_t)lseek(fd,0,SEEK_END);
    map=(uint8_t*)mmap(NULL,fsize,PROT_READ,MAP_PRIVATE,fd,0);::close(fd);if(map==MAP_FAILED){map=nullptr;return false;}
    memcpy(&hs,map,8);std::string jstr((const char*)(map+8),hs);
    json j;try{j=json::parse(jstr);}catch(...){return false;}
    for(auto&[key,val]:j.items()){if(key=="__metadata__")continue;
        if(!val.contains("data_offsets")||!val.contains("shape"))continue;
        size_t stt=val["data_offsets"][0].get<size_t>();auto&sh=val["shape"];if(!sh.is_array())continue;
...
struct WT{std::unordered_map<std::string,struct ggml_tensor*>t;struct ggml_context*ctx=nullptr;
    ggml_backend_buffer_t buf=nullptr;~WT(){if(buf)ggml_backend_buffer_free(buf);if(ctx)ggml_free(ctx);}
...
static struct ggml_tensor*gn_silu(struct ggml_context*c,struct ggml_tensor*x,int grp,const std::string&n,WT&w){
    return ggml_silu(c,ggml_group_norm(c,x,grp,1e-6f));}
...
sd_ctx_t*new_sd_ctx(const sd_ctx_params_t*p){
    ...
    ggml_init_params pp={ctx_size,NULL,false};c->w.ctx=ggml_init(pp);if(!c->w.ctx){delete c;return nullptr;}
    ...
    if(!acp)for(auto&[n,t]:c->w.t){if(n.find("alphas_cumprod")!=std::string::npos){float tmp[1000];
        auto*s=(ggml_fp16_t*)t->data;for(int j=0;j<1000;j++)tmp[j]=ggml_fp16_to_fp32(s[j]);c->den.init(tmp);break;}}
