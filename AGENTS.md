...
- `remote_server.md`：完整远程部署说明。三条路线——**A** `comfycli-bin` workflow（`comfycli_remote.sh` 一键）；**B** `img_hires` 出图管线手动部署（编译命令、scp 文件清单、系统 so 闭包配方、裸机清理）；**C** `sd-cli` 图片编辑（`cpp/sd/edit.sh`，Qwen-Image-2.1 指令编辑，FA+EasyCache 加速 330s→25s）。只跑 `backup*.sh` 出图用 B，图片编辑用 C。
