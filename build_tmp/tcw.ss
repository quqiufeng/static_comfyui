...
(define (static_main )
  (begin
    (set! wgt (static_load_bin "/tmp/sd_weights/vae/decoder_conv_in_weight.bin" (* (* (* 512 16) 3) 3)))
    (set! bias (static_load_bin "/tmp/sd_weights/vae/decoder_conv_in_bias.bin" 512))
    (set! x (make_float_array (* (* (* 1 16) 8) 8)))
    (static_arr_fill x 0.5 (* (* (* 1 16) 8) 8))
    (print "Testing conv2d_inline wrapper...")
    (set! y (static_conv2d_inline x wgt bias 1 16 512 8 8))
    (print "sum=")
    (print (static_arr_sum y (* 64 512)))
    (exit_program 0)
  ))
