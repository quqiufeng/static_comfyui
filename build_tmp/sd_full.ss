...
(define (static_timestep_embedding t dim max_period)
  (begin
    "Sinusoidal 时间步嵌入，与 DDPM 一致"
    (set! emb (make_float_array dim))
    (set! half (quotient dim 2))
    (set! i 0)
    (let loop () (if (< i half) (begin (set! freq (exp (* i (/ (- (log max_period)) half)))) (float_array_set emb i (sin (* t freq))) (float_array_set emb (+ half i) (cos (* t freq))) (set! i (+ i 1)) (loop))))
    (if (= (mod dim 2) 1)
    (begin (float_array_set emb (- dim 1) 0.0)))
    emb
  ))
(define (static_timestep_embedding_batch ts dim n max_period)
  (begin
    "批量时间步嵌入: [n, dim]"
    (set! emb (make_float_array (* n dim)))
    (set! i 0)
    (let loop () (if (< i n) (begin (set! single (static_timestep_embedding (float_array_ref ts i) dim max_period)) (set! j 0) (let loop () (if (< j dim) (begin (float_array_set emb (+ (* i dim) j) (float_array_ref single j)) (set! j (+ j 1)) (loop)))) (set! i (+ i 1)) (loop))))
    emb
  ))
(define (static_resblock_forward x emb w_in_gn1 b_in_gn1 w_in_conv1 b_in_conv1 w_emb_linear b_emb_linear w_out_gn b_out_gn w_out_conv b_out_conv w_skip b_skip n c_in c_out h w emb_dim use_skip)
  (begin
    "ResBlock 前向:
    x → GN → SiLU → Conv2d → + emb_proj → GN → SiLU → Conv2d → + skip(x)
    emb → SiLU → Linear → (scale, shift) → 注入
    "
    (set! h1 (make_float_array (* (* (* n c_in) h) w)))
    (static_arr_copy h1 x (* (* (* n c_in) h) w))
    (static_group_norm h1 w_in_gn1 b_in_gn1 32 c_in (* h w))
    (static_arr_silu h1 h1 (* (* (* n c_in) h) w))
    (set! h1 (static_conv2d h1 w_in_conv1 b_in_conv1 n c_in c_out h w 3 1 1))
    (set! emb_proj (make_float_array emb_dim))
    (static_arr_copy emb_proj emb emb_dim)
    (static_arr_silu emb_proj emb_proj emb_dim)
    (set! emb_out (make_float_array (* 2 c_out)))
    (dgemm_row_auto 1 (* 2 c_out) emb_dim 1.0 emb_proj w_emb_linear 0.0 emb_out)
    (set! i 0)
    (let loop () (if (< i (* 2 c_out)) (begin (float_array_set emb_out i (+ (float_array_ref emb_out i) (float_array_ref b_emb_linear i))) (set! i (+ i 1)) (loop))))
    (set! scale (make_float_array c_out))
    (set! shift (make_float_array c_out))
    (set! i 0)
    (let loop () (if (< i c_out) (begin (float_array_set scale i (float_array_ref emb_out i)) (float_array_set shift i (float_array_ref emb_out (+ c_out i))) (set! i (+ i 1)) (loop))))
    (set! total (* (* (* n c_out) h) w))
    (set! h2 (make_float_array total))
    (static_arr_copy h2 h1 total)
    (static_group_norm h2 w_out_gn b_out_gn 32 c_out (* h w))
    (set! batch 0)
    (let loop () (if (< batch n) (begin (set! ch 0) (let loop () (if (< ch c_out) (begin (set! s (float_array_ref scale ch)) (set! sh (float_array_ref shift ch)) (set! i 0) (let loop () (if (< i (* h w)) (begin (set! idx (+ (* (* (+ (* batch c_out) ch) h) w) i)) (set! v (float_array_ref h2 idx)) (float_array_set h2 idx (+ (* v s) sh)) (set! i (+ i 1)) (loop)))) (set! ch (+ ch 1)) (loop)))) (set! batch (+ batch 1)) (loop))))
    (static_arr_silu h2 h2 total)
    (set! h2 (static_conv2d h2 w_out_conv b_out_conv n c_out c_out h w 3 1 1))
    (if use_skip
    (begin (set! skip (static_conv2d x w_skip b_skip n c_in c_out h w 1 1 0)) (set! i 0) (let loop () (if (< i total) (begin (float_array_set h2 i (+ (float_array_ref h2 i) (float_array_ref skip i))) (set! i (+ i 1)) (loop))))))
    h2
  ))
(define (static_spatial_transformer_forward x context w_gn b_gn w_proj_in b_proj_in w_proj_out b_proj_out attn_params n c h w_in heads dim_head has_context)
  (begin
    "SpatialTransformer:
    x → GN → Conv2d(1x1, c → inner_dim) → reshape [b, t, d] → Attention × depth → reshape back → Conv2d(1x1) + skip
    "
    (set! inner_dim (* heads dim_head))
    (set! total (* (* (* n c) h) w_in))
    (set! h_norm (make_float_array total))
    (static_arr_copy h_norm x total)
    (static_group_norm h_norm w_gn b_gn 32 c (* h w_in))
    (set! h_proj (static_conv2d h_norm w_proj_in b_proj_in n c inner_dim h w_in 1 1 0))
    (set! tokens (* h w_in))
    (set! h_flat (make_float_array (* (* n tokens) inner_dim)))
    (set! batch 0)
    (let loop () (if (< batch n) (begin (set! ch 0) (let loop () (if (< ch inner_dim) (begin (set! i 0) (let loop () (if (< i tokens) (begin (set! src (+ (* (* (+ (* batch inner_dim) ch) h) w_in) i)) (set! dst (+ (* (+ (* batch tokens) i) inner_dim) ch)) (float_array_set h_flat dst (float_array_ref h_proj src)) (set! i (+ i 1)) (loop)))) (set! ch (+ ch 1)) (loop)))) (set! batch (+ batch 1)) (loop))))
    (if (void)
    (begin (set! out (static_cross_attention h_flat context context n tokens tokens inner_dim heads)))
    (begin (set! out (static_attention_sd h_flat h_flat h_flat n tokens tokens inner_dim heads))))
    (set! h_back (make_float_array (* (* (* n inner_dim) h) w_in)))
    (set! batch 0)
    (let loop () (if (< batch n) (begin (set! ch 0) (let loop () (if (< ch inner_dim) (begin (set! i 0) (let loop () (if (< i tokens) (begin (set! src (+ (* (+ (* batch tokens) i) inner_dim) ch)) (set! dst (+ (* (* (+ (* batch inner_dim) ch) h) w_in) i)) (float_array_set h_back dst (float_array_ref out src)) (set! i (+ i 1)) (loop)))) (set! ch (+ ch 1)) (loop)))) (set! batch (+ batch 1)) (loop))))
    (set! out_final (static_conv2d h_back w_proj_out b_proj_out n inner_dim c h w_in 1 1 0))
    (set! i 0)
    (let loop () (if (< i total) (begin (float_array_set out_final i (+ (float_array_ref out_final i) (float_array_ref x i))) (set! i (+ i 1)) (loop))))
    out_final
  ))
(define (static_down_block_forward x emb context resnet_params transformer_params downsample_params n c_in c_out h w emb_dim has_attn has_downsample)
  (begin
    "下采样块: ResBlock × N → [Attention] → Downsample"
    (set! h_current (make_float_array (* (* (* n c_in) h) w)))
    (static_arr_copy h_current x (* (* (* n c_in) h) w))
    (set! h_current (static_resblock_forward h_current emb (float_array_ref resnet_params 0) (float_array_ref resnet_params 1) (float_array_ref resnet_params 2) (float_array_ref resnet_params 3) (float_array_ref resnet_params 4) (float_array_ref resnet_params 5) (float_array_ref resnet_params 6) (float_array_ref resnet_params 7) (float_array_ref resnet_params 8) (float_array_ref resnet_params 9) (float_array_ref resnet_params 10) (float_array_ref resnet_params 11) n c_in c_out h w emb_dim (if (not (= c_in c_out)) 1 0)))
    (if has_attn
    (begin (set! h_current (static_spatial_transformer_forward h_current context (float_array_ref transformer_params 0) (float_array_ref transformer_params 1) (float_array_ref transformer_params 2) (float_array_ref transformer_params 3) (float_array_ref transformer_params 4) (float_array_ref transformer_params 5) #f n c_out h w 8 64 1))))
    (if has_downsample
    (begin (set! h_out (quotient h 2)) (set! w_out (quotient w 2)) (set! h_current (static_conv2d h_current (float_array_ref downsample_params 0) (float_array_ref downsample_params 1) n c_out c_out h w 3 2 1))))
    h_current
  ))
(define (static_up_block_forward x skip emb context resnet_params transformer_params upsample_params n c_in c_out h w emb_dim has_attn has_upsample)
  (begin
    "上采样块: Upsample → ResBlock × N → [Attention]"
    (set! h_current (make_float_array (* (* (* n c_in) h) w)))
    (static_arr_copy h_current x (* (* (* n c_in) h) w))
    (if has_upsample
    (begin (set! h_current (static_upsample_nearest h_current n c_in h w 2)) (set! h (* h 2)) (set! w (* w 2)) (set! h_current (static_conv2d h_current (float_array_ref upsample_params 0) (float_array_ref upsample_params 1) n c_in c_in h w 3 1 1))))
    (set! total_skip (* (* (* n c_out) h) w))
    (set! total_concat (* total_skip 2))
    (set! concat (make_float_array total_concat))
    (set! i 0)
    (let loop () (if (< i total_skip) (begin (float_array_set concat i (float_array_ref h_current i)) (float_array_set concat (+ total_skip i) (float_array_ref skip i)) (set! i (+ i 1)) (loop))))
    (set! h_current (static_resblock_forward concat emb (float_array_ref resnet_params 0) (float_array_ref resnet_params 1) (float_array_ref resnet_params 2) (float_array_ref resnet_params 3) (float_array_ref resnet_params 4) (float_array_ref resnet_params 5) (float_array_ref resnet_params 6) (float_array_ref resnet_params 7) (float_array_ref resnet_params 8) (float_array_ref resnet_params 9) (float_array_ref resnet_params 10) (float_array_ref resnet_params 11) n (* c_out 2) c_out h w emb_dim 1))
    (if has_attn
    (begin (set! h_current (static_spatial_transformer_forward h_current context (float_array_ref transformer_params 0) (float_array_ref transformer_params 1) (float_array_ref transformer_params 2) (float_array_ref transformer_params 3) (float_array_ref transformer_params 4) (float_array_ref transformer_params 5) #f n c_out h w 8 64 1))))
    h_current
  ))
(define (static_make_betas n_steps beta_start beta_end)
  (begin
    "线性 beta 调度"
    (set! betas (make_float_array n_steps))
    (set! i 0)
    (let loop () (if (< i n_steps) (begin (float_array_set betas i (+ beta_start (/ (* (- beta_end beta_start) i) (- n_steps 1)))) (set! i (+ i 1)) (loop))))
    betas
  ))
(define (static_make_alphas betas n)
  (begin
    "alpha = 1 - beta, alpha_bar = cumprod(alpha)"
    (set! alphas (make_float_array n))
    (set! alpha_bars (make_float_array n))
    (set! cumprod 1.0)
    (set! i 0)
    (let loop () (if (< i n) (begin (set! a (- 1.0 (float_array_ref betas i))) (set! cumprod (* cumprod a)) (float_array_set alphas i a) (float_array_set alpha_bars i cumprod) (set! i (+ i 1)) (loop))))
    alpha_bars
  ))
(define (static_cfg_apply uncond_pred cond_pred cfg_scale n)
  (begin
    "CFG: pred = uncond + cfg * (cond - uncond)"
    (set! i 0)
    (let loop () (if (< i n) (begin (set! uc (float_array_ref uncond_pred i)) (set! c (float_array_ref cond_pred i)) (float_array_set cond_pred i (+ uc (* cfg_scale (- c uc)))) (set! i (+ i 1)) (loop))))
  ))
(define (static_ddim_step x pred_noise alpha_bar_t alpha_bar_prev n)
  (begin
    "DDIM 单步: x_{t-1} = sqrt(alpha_bar_prev) * pred_x0 + sqrt(1 - alpha_bar_prev) * eps
    pred_x0 = (x_t - sqrt(1 - alpha_bar_t) * eps) / sqrt(alpha_bar_t)
    "
    (set! sqrt_ab_t (sqrt alpha_bar_t))
    (set! sqrt_1_ab_t (sqrt (- 1.0 alpha_bar_t)))
    (set! sqrt_ab_prev (sqrt alpha_bar_prev))
    (set! sqrt_1_ab_prev (sqrt (- 1.0 alpha_bar_prev)))
    (set! pred_x0 (make_float_array n))
    (set! i 0)
    (let loop () (if (< i n) (begin (set! v (/ (- (float_array_ref x i) (* sqrt_1_ab_t (float_array_ref pred_noise i))) sqrt_ab_t)) (float_array_set pred_x0 i v) (set! i (+ i 1)) (loop))))
    (set! x_prev (make_float_array n))
    (set! i 0)
    (let loop () (if (< i n) (begin (set! v (+ (* sqrt_ab_prev (float_array_ref pred_x0 i)) (* sqrt_1_ab_prev (float_array_ref pred_noise i)))) (float_array_set x_prev i v) (set! i (+ i 1)) (loop))))
    x_prev
  ))
(define (static_euler_step x pred_noise sigma sigma_next n)
  (begin
    "Euler 单步: x_{t-1} = x_t + (sigma_next - sigma) * pred_noise"
    (set! dt (- sigma_next sigma))
    (set! x_next (make_float_array n))
    (set! i 0)
    (let loop () (if (< i n) (begin (float_array_set x_next i (+ (float_array_ref x i) (* dt (float_array_ref pred_noise i)))) (set! i (+ i 1)) (loop))))
    x_next
  ))
(define (static_dpmpp_2m_step x pred_noise sigma sigma_next sigma_prev n old_denoised)
  (begin
    "DPM++ 2M 单步（二阶）"
    (set! dt (- sigma_next sigma))
    (set! x_next (make_float_array n))
...
(define (static_sample_ddim unet_fn latent context uncond_context n c h w steps cfg_scale)
  (begin
    "DDIM 采样循环"
    (set! total (* (* (* n c) h) w))
    (set! x (make_float_array total))
    (static_arr_copy x latent total)
    (set! betas (static_make_betas steps 0.00085 0.012))
    (set! alpha_bars (static_make_alphas betas steps))
    (set! t (- steps 1))
    (let loop () (if (>= t 0) (begin (set! ab_t (float_array_ref alpha_bars t)) (set! ab_prev (if (> t 0) (float_array_ref alpha_bars (- t 1)) 1.0)) (set! ts (make_float_array n)) (static_arr_fill ts t n) (set! pred_uncond (static_unet_fn x ts uncond_context n c h w)) (set! pred_cond (static_unet_fn x ts context n c h w)) (static_cfg_apply pred_uncond pred_cond cfg_scale total) (set! x (static_ddim_step x pred_cond ab_t ab_prev total)) (set! t (- t 1)) (loop))))
    x
  ))
(define (static_clip_tokenize text max_len)
  (begin
    "简化版 CLIP tokenizer: 将文本映射为 token IDs
    注: 完整版需要 BPE 词表，这里简化为固定映射
    "
    (set! tokens (make_float_array max_len))
    (static_arr_fill tokens 0.0 max_len)
    (float_array_set tokens 0 49406.0)
    (float_array_set tokens (- max_len 1) 49407.0)
    tokens
  ))
(define (static_clip_transformer_forward tokens w_token_embed w_pos_embed transformer_params n seq_len dim)
  (begin
    ...
    "
    (set! hidden (make_float_array (* (* n seq_len) dim)))
    (set! batch 0)
    (let loop () (if (< batch n) (begin (set! pos 0) (let loop () (if (< pos seq_len) (begin (set! tok_id (float_array_ref tokens (+ (* batch seq_len) pos))) (set! i 0) (let loop () (if (< i dim) (begin (set! emb_val (float_array_ref w_token_embed (+ (* (exact (round tok_id)) dim) i))) (set! pos_val (float_array_ref w_pos_embed (+ (* pos dim) i))) (float_array_set hidden (+ (* (+ (* batch seq_len) pos) dim) i) (+ emb_val pos_val)) (set! i (+ i 1)) (loop)))) (set! pos (+ pos 1)) (loop)))) (set! batch (+ batch 1)) (loop))))
    (set! out (make_float_array (* (* n seq_len) dim)))
    (static_arr_copy out hidden (* (* n seq_len) dim))
    out
  ))
(define (static_clip_encode text model_params n seq_len dim)
  (begin
    "完整 CLIP 编码管线: tokenize → transformer → pool"
    (set! tokens (static_clip_tokenize text seq_len))
    (set! embeddings (static_clip_transformer_forward tokens model_params model_params #f n seq_len dim))
    embeddings
  ))
...
(define (static_weights_exist dir)
  (begin
    "检查权重文件是否存在"
    (set! fp (fopen (string-append dir "/index.json") "rb"))
    (if (= fp 0)
    (begin 0))
    (fclose fp)
    1
  ))
(define (static_load_weight_by_name weight_dir name n)
  (begin
    "按名称加载权重: {dir}/{safe_name}.bin"
    (set! safe name)
    (set! i 0)
    (let loop () (if (< i 100) (begin (set! i (+ i 1)) (loop))))
    (static_load_bin (string-append (+ (string-append weight_dir "/") safe) ".bin") n)
  ))
(define (static_resblock_forward x emb w_norm1 b_norm1 w_conv1 b_conv1 w_norm2 b_norm2 w_conv2 b_conv2 w_emb b_emb w_skip b_skip n c_in c_out hh ww)
  (begin
    "ResBlock: GN→SiLU→Conv→+temb→GN→SiLU→Conv→+skip"
    (static_group_norm x w_norm1 b_norm1 32 c_in (* hh ww))
    (static_arr_silu x x (* (* (* n c_in) hh) ww))
    (set! x (static_conv2d_inline x w_conv1 b_conv1 n c_in c_out hh ww))
    (set! _y (make_float_array (* (* n c_out) 2)))
    (dgemm_row_auto n (* c_out 2) 1280 1.0 emb w_emb 0.0 _y)
    (static_add_bias _y b_emb n (* c_out 2))
    (static_apply_scale_shift x _y n c_out (* hh ww))
    (static_group_norm x w_norm2 b_norm2 32 c_out (* hh ww))
    (static_arr_silu x x (* (* (* n c_out) hh) ww))
    (set! x (static_conv2d_inline x w_conv2 b_conv2 n c_out c_out hh ww))
    (if (not (= c_in c_out))
    (begin (set! _skip (static_conv2d_inline x_orig w_skip b_skip n c_in c_out hh ww)) (static_add_arr x _skip (* (* (* n c_out) hh) ww))))
    x
  ))
(define (static_transformer_forward x context w_norm b_norm w_proj_in b_proj_in w_proj_out b_proj_out attn_k_weight attn_k_bias attn_q_weight attn_q_bias attn_v_weight attn_v_bias attn_out_weight attn_out_bias n c hh ww d_head)
  (begin
    "SpatialTransformer: GN → 1x1Conv → reshape → Attention → reshape → 1x1Conv → +x"
    (set! _inner c)
    (set! _total (* (* (* n c) hh) ww))
    (set! _src (make_float_array _total))
    (static_arr_copy _src x _total)
    (static_group_norm x w_norm b_norm 32 c (* hh ww))
    (set! x (static_conv2d_inline x w_proj_in b_proj_in n c _inner hh ww))
    (set! _tokens (* hh ww))
    (set! _h_flat (make_float_array (* (* n _tokens) _inner)))
    (set! _b 0)
    (let loop () (if (< _b n) (begin (set! _ch 0) (let loop () (if (< _ch _inner) (begin (set! _t 0) (let loop () (if (< _t _tokens) (begin (float_array_set _h_flat (+ (* (+ (* _b _tokens) _t) _inner) _ch) (float_array_ref x (+ (* (* (+ (* _b _inner) _ch) hh) ww) _t))) (set! _t (+ _t 1)) (loop)))) (set! _ch (+ _ch 1)) (loop)))) (set! _b (+ _b 1)) (loop))))
    (set! _dim _inner)
    (set! _q (make_float_array (* (* n _tokens) _dim)))
    (dgemm_row_auto (* n _tokens) _dim _dim 1.0 _h_flat attn_q_weight 0.0 _q)
    (static_add_bias _q attn_q_bias (* n _tokens) _dim)
    (set! _k (make_float_array (* (* n _tokens) _dim)))
    (dgemm_row_auto (* n _tokens) _dim _dim 1.0 _h_flat attn_k_weight 0.0 _k)
    (static_add_bias _k attn_k_bias (* n _tokens) _dim)
    (set! _v (make_float_array (* (* n _tokens) _dim)))
    (dgemm_row_auto (* n _tokens) _dim _dim 1.0 _h_flat attn_v_weight 0.0 _v)
    (static_add_bias _v attn_v_bias (* n _tokens) _dim)
    (set! _out (static_attention_sd _q _k _v n _tokens _tokens _dim (quotient _dim d_head)))
    (set! _out2 (make_float_array (* (* n _tokens) _dim)))
    (dgemm_row_auto (* n _tokens) _dim _dim 1.0 _out attn_out_weight 0.0 _out2)
    (static_add_bias _out2 attn_out_bias (* n _tokens) _dim)
    (set! _h_back (make_float_array _total))
    (set! _b 0)
    (let loop () (if (< _b n) (begin (set! _ch 0) (let loop () (if (< _ch _dim) (begin (set! _t 0) (let loop () (if (< _t _tokens) (begin (float_array_set _h_back (+ (* (* (+ (* _b _dim) _ch) hh) ww) _t) (float_array_ref _out2 (+ (* (+ (* _b _tokens) _t) _dim) _ch))) (set! _t (+ _t 1)) (loop)))) (set! _ch (+ _ch 1)) (loop)))) (set! _b (+ _b 1)) (loop))))
    (static_add_arr _h_back _src _total)
    _h_back
  ))
(define (static_apply_scale_shift x ss n c spatial)
  (begin
...
(define (static_downsample_conv x w b n c hh ww)
  (begin
    "直接 conv2d stride=2"
    (static_conv2d_inline x w b n c c hh ww)
  ))
(define (static_vae_decoder_forward latent weights_dir n height width)
  (begin
    (set! h_cur #f)
    (set! hh height)
    (set! ww width)
    (set! conv_in_bias (static_load_bin (string-append weights_dir "/decoder_conv_in_bias.bin") 512))
    (set! conv_in_weight (static_load_bin (string-append weights_dir "/decoder_conv_in_weight.bin") 73728))
    (set! conv_out_bias (static_load_bin (string-append weights_dir "/decoder_conv_out_bias.bin") 3))
    (set! conv_out_weight (static_load_bin (string-append weights_dir "/decoder_conv_out_weight.bin") 3456))
    (set! mid_attn_1_k_bias (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_k_bias.bin") 512))
    (set! mid_attn_1_k_weight (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_k_weight.bin") 262144))
    (set! mid_attn_1_norm_bias (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_norm_bias.bin") 512))
    (set! mid_attn_1_norm_weight (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_norm_weight.bin") 512))
    (set! mid_attn_1_proj_out_bias (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_proj_out_bias.bin") 512))
    (set! mid_attn_1_proj_out_weight (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_proj_out_weight.bin") 262144))
    (set! mid_attn_1_q_bias (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_q_bias.bin") 512))
    (set! mid_attn_1_q_weight (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_q_weight.bin") 262144))
    (set! mid_attn_1_v_bias (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_v_bias.bin") 512))
    (set! mid_attn_1_v_weight (static_load_bin (string-append weights_dir "/decoder_mid_attn_1_v_weight.bin") 262144))
    (set! mid_block_1_conv1_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_1_conv1_bias.bin") 512))
    (set! mid_block_1_conv1_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_1_conv1_weight.bin") 2359296))
    (set! mid_block_1_conv2_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_1_conv2_bias.bin") 512))
    (set! mid_block_1_conv2_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_1_conv2_weight.bin") 2359296))
    (set! mid_block_1_norm1_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_1_norm1_bias.bin") 512))
    (set! mid_block_1_norm1_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_1_norm1_weight.bin") 512))
    (set! mid_block_1_norm2_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_1_norm2_bias.bin") 512))
    (set! mid_block_1_norm2_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_1_norm2_weight.bin") 512))
    (set! mid_block_2_conv1_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_2_conv1_bias.bin") 512))
    (set! mid_block_2_conv1_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_2_conv1_weight.bin") 2359296))
    (set! mid_block_2_conv2_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_2_conv2_bias.bin") 512))
    (set! mid_block_2_conv2_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_2_conv2_weight.bin") 2359296))
    (set! mid_block_2_norm1_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_2_norm1_bias.bin") 512))
    (set! mid_block_2_norm1_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_2_norm1_weight.bin") 512))
    (set! mid_block_2_norm2_bias (static_load_bin (string-append weights_dir "/decoder_mid_block_2_norm2_bias.bin") 512))
    (set! mid_block_2_norm2_weight (static_load_bin (string-append weights_dir "/decoder_mid_block_2_norm2_weight.bin") 512))
    (set! norm_out_bias (static_load_bin (string-append weights_dir "/decoder_norm_out_bias.bin") 128))
    (set! norm_out_weight (static_load_bin (string-append weights_dir "/decoder_norm_out_weight.bin") 128))
    (set! up_0_block_0_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_conv1_bias.bin") 128))
    (set! up_0_block_0_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_conv1_weight.bin") 294912))
    (set! up_0_block_0_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_conv2_bias.bin") 128))
    (set! up_0_block_0_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_conv2_weight.bin") 147456))
    (set! up_0_block_0_nin_shortcut_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_nin_shortcut_bias.bin") 128))
    (set! up_0_block_0_nin_shortcut_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_nin_shortcut_weight.bin") 32768))
    (set! up_0_block_0_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_norm1_bias.bin") 256))
    (set! up_0_block_0_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_norm1_weight.bin") 256))
    (set! up_0_block_0_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_norm2_bias.bin") 128))
    (set! up_0_block_0_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_0_norm2_weight.bin") 128))
    (set! up_0_block_1_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_conv1_bias.bin") 128))
    (set! up_0_block_1_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_conv1_weight.bin") 147456))
    (set! up_0_block_1_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_conv2_bias.bin") 128))
    (set! up_0_block_1_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_conv2_weight.bin") 147456))
    (set! up_0_block_1_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_norm1_bias.bin") 128))
    (set! up_0_block_1_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_norm1_weight.bin") 128))
    (set! up_0_block_1_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_norm2_bias.bin") 128))
    (set! up_0_block_1_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_1_norm2_weight.bin") 128))
    (set! up_0_block_2_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_conv1_bias.bin") 128))
    (set! up_0_block_2_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_conv1_weight.bin") 147456))
    (set! up_0_block_2_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_conv2_bias.bin") 128))
    (set! up_0_block_2_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_conv2_weight.bin") 147456))
    (set! up_0_block_2_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_norm1_bias.bin") 128))
    (set! up_0_block_2_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_norm1_weight.bin") 128))
    (set! up_0_block_2_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_norm2_bias.bin") 128))
    (set! up_0_block_2_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_0_block_2_norm2_weight.bin") 128))
    (set! up_1_block_0_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_conv1_bias.bin") 256))
    (set! up_1_block_0_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_conv1_weight.bin") 1179648))
    (set! up_1_block_0_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_conv2_bias.bin") 256))
    (set! up_1_block_0_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_conv2_weight.bin") 589824))
    (set! up_1_block_0_nin_shortcut_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_nin_shortcut_bias.bin") 256))
    (set! up_1_block_0_nin_shortcut_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_nin_shortcut_weight.bin") 131072))
    (set! up_1_block_0_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_norm1_bias.bin") 512))
    (set! up_1_block_0_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_norm1_weight.bin") 512))
    (set! up_1_block_0_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_norm2_bias.bin") 256))
    (set! up_1_block_0_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_0_norm2_weight.bin") 256))
    (set! up_1_block_1_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_conv1_bias.bin") 256))
    (set! up_1_block_1_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_conv1_weight.bin") 589824))
    (set! up_1_block_1_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_conv2_bias.bin") 256))
    (set! up_1_block_1_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_conv2_weight.bin") 589824))
    (set! up_1_block_1_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_norm1_bias.bin") 256))
    (set! up_1_block_1_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_norm1_weight.bin") 256))
    (set! up_1_block_1_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_norm2_bias.bin") 256))
    (set! up_1_block_1_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_1_norm2_weight.bin") 256))
    (set! up_1_block_2_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_conv1_bias.bin") 256))
    (set! up_1_block_2_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_conv1_weight.bin") 589824))
    (set! up_1_block_2_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_conv2_bias.bin") 256))
    (set! up_1_block_2_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_conv2_weight.bin") 589824))
    (set! up_1_block_2_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_norm1_bias.bin") 256))
    (set! up_1_block_2_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_norm1_weight.bin") 256))
    (set! up_1_block_2_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_norm2_bias.bin") 256))
    (set! up_1_block_2_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_1_block_2_norm2_weight.bin") 256))
    (set! up_1_upsample_conv_bias (static_load_bin (string-append weights_dir "/decoder_up_1_upsample_conv_bias.bin") 256))
    (set! up_1_upsample_conv_weight (static_load_bin (string-append weights_dir "/decoder_up_1_upsample_conv_weight.bin") 589824))
    (set! up_2_block_0_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_conv1_bias.bin") 512))
    (set! up_2_block_0_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_conv1_weight.bin") 2359296))
    (set! up_2_block_0_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_conv2_bias.bin") 512))
    (set! up_2_block_0_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_conv2_weight.bin") 2359296))
    (set! up_2_block_0_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_norm1_bias.bin") 512))
    (set! up_2_block_0_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_norm1_weight.bin") 512))
    (set! up_2_block_0_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_norm2_bias.bin") 512))
    (set! up_2_block_0_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_0_norm2_weight.bin") 512))
    (set! up_2_block_1_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_conv1_bias.bin") 512))
    (set! up_2_block_1_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_conv1_weight.bin") 2359296))
    (set! up_2_block_1_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_conv2_bias.bin") 512))
    (set! up_2_block_1_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_conv2_weight.bin") 2359296))
    (set! up_2_block_1_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_norm1_bias.bin") 512))
    (set! up_2_block_1_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_norm1_weight.bin") 512))
    (set! up_2_block_1_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_norm2_bias.bin") 512))
    (set! up_2_block_1_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_1_norm2_weight.bin") 512))
    (set! up_2_block_2_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_conv1_bias.bin") 512))
    (set! up_2_block_2_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_conv1_weight.bin") 2359296))
    (set! up_2_block_2_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_conv2_bias.bin") 512))
    (set! up_2_block_2_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_conv2_weight.bin") 2359296))
    (set! up_2_block_2_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_norm1_bias.bin") 512))
    (set! up_2_block_2_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_norm1_weight.bin") 512))
    (set! up_2_block_2_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_norm2_bias.bin") 512))
    (set! up_2_block_2_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_2_block_2_norm2_weight.bin") 512))
    (set! up_2_upsample_conv_bias (static_load_bin (string-append weights_dir "/decoder_up_2_upsample_conv_bias.bin") 512))
    (set! up_2_upsample_conv_weight (static_load_bin (string-append weights_dir "/decoder_up_2_upsample_conv_weight.bin") 2359296))
    (set! up_3_block_0_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_conv1_bias.bin") 512))
    (set! up_3_block_0_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_conv1_weight.bin") 2359296))
    (set! up_3_block_0_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_conv2_bias.bin") 512))
    (set! up_3_block_0_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_conv2_weight.bin") 2359296))
    (set! up_3_block_0_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_norm1_bias.bin") 512))
    (set! up_3_block_0_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_norm1_weight.bin") 512))
    (set! up_3_block_0_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_norm2_bias.bin") 512))
    (set! up_3_block_0_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_0_norm2_weight.bin") 512))
    (set! up_3_block_1_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_conv1_bias.bin") 512))
    (set! up_3_block_1_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_conv1_weight.bin") 2359296))
    (set! up_3_block_1_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_conv2_bias.bin") 512))
    (set! up_3_block_1_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_conv2_weight.bin") 2359296))
    (set! up_3_block_1_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_norm1_bias.bin") 512))
    (set! up_3_block_1_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_norm1_weight.bin") 512))
    (set! up_3_block_1_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_norm2_bias.bin") 512))
    (set! up_3_block_1_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_1_norm2_weight.bin") 512))
    (set! up_3_block_2_conv1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_conv1_bias.bin") 512))
    (set! up_3_block_2_conv1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_conv1_weight.bin") 2359296))
    (set! up_3_block_2_conv2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_conv2_bias.bin") 512))
    (set! up_3_block_2_conv2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_conv2_weight.bin") 2359296))
    (set! up_3_block_2_norm1_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_norm1_bias.bin") 512))
    (set! up_3_block_2_norm1_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_norm1_weight.bin") 512))
    (set! up_3_block_2_norm2_bias (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_norm2_bias.bin") 512))
    (set! up_3_block_2_norm2_weight (static_load_bin (string-append weights_dir "/decoder_up_3_block_2_norm2_weight.bin") 512))
    (set! up_3_upsample_conv_bias (static_load_bin (string-append weights_dir "/decoder_up_3_upsample_conv_bias.bin") 512))
    (set! up_3_upsample_conv_weight (static_load_bin (string-append weights_dir "/decoder_up_3_upsample_conv_weight.bin") 2359296))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    (set! _nc (* (* 1 _ho) _wo))
    (set! _kd (* (* 16 3) 3))
    (print "alloc col2...")
    (set! _col (make_float_array (* _nc _kd)))
    (static_im2col latent 1 16 hh ww 3 1 1 _col)
    (print "alloc y2...")
    (set! _y (make_float_array (* _nc 512)))
    (dgemm_row_auto _nc 512 _kd 1.0 _col conv_in_weight 0.0 _y)
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref conv_in_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 1")
    (static_group_norm h_cur mid_block_1_norm1_weight mid_block_1_norm1_bias 32 512 (* hh ww))
    (print "step 2")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    (set! _nc (* (* 1 _ho) _wo))
    (set! _kd (* (* 512 3) 3))
    ...
    (set! _col (make_float_array (* _nc _kd)))
    (print "im2col2...")
    (print "mid_conv_start...")
    (static_im2col h_cur 1 512 hh ww 3 1 1 _col)
    (set! _y (make_float_array (* _nc 512)))
    (dgemm_row_auto _nc 512 _kd 1.0 _col mid_block_1_conv1_weight 0.0 _y)
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref mid_block_1_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 3")
    (static_group_norm h_cur mid_block_1_norm2_weight mid_block_1_norm2_bias 32 512 (* hh ww))
    (print "step 4")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref mid_block_1_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 5")
    (static_group_norm h_cur mid_block_2_norm1_weight mid_block_2_norm1_bias 32 512 (* hh ww))
    (print "step 6")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref mid_block_2_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 7")
    (static_group_norm h_cur mid_block_2_norm2_weight mid_block_2_norm2_bias 32 512 (* hh ww))
    (print "step 8")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref mid_block_2_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 9")
    (static_group_norm h_cur up_0_block_0_norm1_weight up_0_block_0_norm1_bias 32 256 (* hh ww))
    (print "step 10")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    (set! _nc (* (* 1 _ho) _wo))
    (set! _kd (* (* 256 3) 3))
    (print "alloc col2...")
    (set! _col (make_float_array (* _nc _kd)))
    (static_im2col h_cur 1 256 hh ww 3 1 1 _col)
    (set! _y (make_float_array (* _nc 256)))
    (dgemm_row_auto _nc 256 _kd 1.0 _col up_0_block_0_conv1_weight 0.0 _y)
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_0_block_0_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 11")
    (static_group_norm h_cur up_0_block_0_norm2_weight up_0_block_0_norm2_bias 32 256 (* hh ww))
    (print "step 12")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_0_block_0_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 13")
    (static_group_norm h_cur up_0_block_1_norm1_weight up_0_block_1_norm1_bias 32 128 (* hh ww))
    (print "step 14")
    (static_arr_silu h_cur h_cur (* (* (* 1 128) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    (set! _nc (* (* 1 _ho) _wo))
    (set! _kd (* (* 128 3) 3))
    (print "alloc col2...")
    (set! _col (make_float_array (* _nc _kd)))
    (static_im2col h_cur 1 128 hh ww 3 1 1 _col)
    (set! _y (make_float_array (* _nc 128)))
    (dgemm_row_auto _nc 128 _kd 1.0 _col up_0_block_1_conv1_weight 0.0 _y)
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 128) (begin (float_array_set _y (+ (* __i 128) __j) (+ (float_array_ref _y (+ (* __i 128) __j)) (float_array_ref up_0_block_1_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 15")
    (static_group_norm h_cur up_0_block_1_norm2_weight up_0_block_1_norm2_bias 32 128 (* hh ww))
    (print "step 16")
    (static_arr_silu h_cur h_cur (* (* (* 1 128) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 128) (begin (float_array_set _y (+ (* __i 128) __j) (+ (float_array_ref _y (+ (* __i 128) __j)) (float_array_ref up_0_block_1_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 17")
    (static_group_norm h_cur up_0_block_2_norm1_weight up_0_block_2_norm1_bias 32 128 (* hh ww))
    (print "step 18")
    (static_arr_silu h_cur h_cur (* (* (* 1 128) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 128) (begin (float_array_set _y (+ (* __i 128) __j) (+ (float_array_ref _y (+ (* __i 128) __j)) (float_array_ref up_0_block_2_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 19")
    (static_group_norm h_cur up_0_block_2_norm2_weight up_0_block_2_norm2_bias 32 128 (* hh ww))
    (print "step 20")
    (static_arr_silu h_cur h_cur (* (* (* 1 128) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 128) (begin (float_array_set _y (+ (* __i 128) __j) (+ (float_array_ref _y (+ (* __i 128) __j)) (float_array_ref up_0_block_2_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 21")
    (set! h_cur (static_upsample_nearest h_cur 1 256 hh ww 2))
    (print "step 22")
    (set! hh (* hh 2))
    (set! ww (* ww 2))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_1_upsample_conv_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 23")
    (static_group_norm h_cur up_1_block_0_norm1_weight up_1_block_0_norm1_bias 32 512 (* hh ww))
    (print "step 24")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_1_block_0_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 25")
    (static_group_norm h_cur up_1_block_0_norm2_weight up_1_block_0_norm2_bias 32 512 (* hh ww))
    (print "step 26")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_1_block_0_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 27")
    (static_group_norm h_cur up_1_block_1_norm1_weight up_1_block_1_norm1_bias 32 256 (* hh ww))
    (print "step 28")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_1_block_1_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 29")
    (static_group_norm h_cur up_1_block_1_norm2_weight up_1_block_1_norm2_bias 32 256 (* hh ww))
    (print "step 30")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_1_block_1_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 31")
    (static_group_norm h_cur up_1_block_2_norm1_weight up_1_block_2_norm1_bias 32 256 (* hh ww))
    (print "step 32")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_1_block_2_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 33")
    (static_group_norm h_cur up_1_block_2_norm2_weight up_1_block_2_norm2_bias 32 256 (* hh ww))
    (print "step 34")
    (static_arr_silu h_cur h_cur (* (* (* 1 256) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 256) (begin (float_array_set _y (+ (* __i 256) __j) (+ (float_array_ref _y (+ (* __i 256) __j)) (float_array_ref up_1_block_2_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 35")
    (set! h_cur (static_upsample_nearest h_cur 1 512 hh ww 2))
    (print "step 36")
    (set! hh (* hh 2))
    (set! ww (* ww 2))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_upsample_conv_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 37")
    (static_group_norm h_cur up_2_block_0_norm1_weight up_2_block_0_norm1_bias 32 512 (* hh ww))
    (print "step 38")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_0_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 39")
    (static_group_norm h_cur up_2_block_0_norm2_weight up_2_block_0_norm2_bias 32 512 (* hh ww))
    (print "step 40")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_0_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 41")
    (static_group_norm h_cur up_2_block_1_norm1_weight up_2_block_1_norm1_bias 32 512 (* hh ww))
    (print "step 42")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_1_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 43")
    (static_group_norm h_cur up_2_block_1_norm2_weight up_2_block_1_norm2_bias 32 512 (* hh ww))
    (print "step 44")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_1_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 45")
    (static_group_norm h_cur up_2_block_2_norm1_weight up_2_block_2_norm1_bias 32 512 (* hh ww))
    (print "step 46")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_2_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 47")
    (static_group_norm h_cur up_2_block_2_norm2_weight up_2_block_2_norm2_bias 32 512 (* hh ww))
    (print "step 48")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_2_block_2_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 49")
    (set! h_cur (static_upsample_nearest h_cur 1 512 hh ww 2))
    (print "step 50")
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_upsample_conv_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 51")
    (static_group_norm h_cur up_3_block_0_norm1_weight up_3_block_0_norm1_bias 32 512 (* hh ww))
    (print "step 52")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_0_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 53")
    (static_group_norm h_cur up_3_block_0_norm2_weight up_3_block_0_norm2_bias 32 512 (* hh ww))
    (print "step 54")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_0_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 55")
    (static_group_norm h_cur up_3_block_1_norm1_weight up_3_block_1_norm1_bias 32 512 (* hh ww))
    (print "step 56")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_1_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 57")
    (static_group_norm h_cur up_3_block_1_norm2_weight up_3_block_1_norm2_bias 32 512 (* hh ww))
    (print "step 58")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_1_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 59")
    (static_group_norm h_cur up_3_block_2_norm1_weight up_3_block_2_norm1_bias 32 512 (* hh ww))
    (print "step 60")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_2_conv1_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 61")
    (static_group_norm h_cur up_3_block_2_norm2_weight up_3_block_2_norm2_bias 32 512 (* hh ww))
    (print "step 62")
    (static_arr_silu h_cur h_cur (* (* (* 1 512) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 512) (begin (float_array_set _y (+ (* __i 512) __j) (+ (float_array_ref _y (+ (* __i 512) __j)) (float_array_ref up_3_block_2_conv2_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 63")
    (static_group_norm h_cur norm_out_weight norm_out_bias 32 128 (* hh ww))
    (print "step 64")
    (static_arr_silu h_cur h_cur (* (* (* 1 128) hh) ww))
    (set! _ho (+ (quotient (- (+ hh (* 2 1)) 3) 1) 1))
    (set! _wo (+ (quotient (- (+ ww (* 2 1)) 3) 1) 1))
    ...
    (set! _col (make_float_array (* _nc _kd)))
    (static_im2col h_cur 1 128 hh ww 3 1 1 _col)
    (set! _y (make_float_array (* _nc 3)))
    (dgemm_row_auto _nc 3 _kd 1.0 _col conv_out_weight 0.0 _y)
    (set! __i 0)
    (let loop () (if (< __i _nc) (begin (set! __j 0) (let loop () (if (< __j 3) (begin (float_array_set _y (+ (* __i 3) __j) (+ (float_array_ref _y (+ (* __i 3) __j)) (float_array_ref conv_out_bias __j))) (set! __j (+ __j 1)) (loop)))) (set! __i (+ __i 1)) (loop))))
    (set! h_cur _y)
    (print "step 65")
    (static_arr_clip h_cur 0.0 1.0 (* (* (* 1 3) hh) ww))
    h_cur
    h_cur
  ))
(define (static_load_bin fn n)
...
(define (static_main )
  (begin
    ...
    (set! wdir "/tmp/sd_weights/vae")
    (set! n 1)
    (set! h 8)
    (set! w 8)
    (set! latent (make_float_array (* (* (* n 16) h) w)))
    (set! i 0)
    (let loop () (if (< i (* (* (* n 16) h) w)) (begin (float_array_set latent i (- (/ (mod (+ (* i 7) 3) 100) 100.0) 0.5)) (set! i (+ i 1)) (loop))))
    (print "Running VAE decoder...")
    (set! image (static_vae_decoder_forward latent wdir n h w))
    (set! oh (* h 8))
    (set! ow (* w 8))
    (print "Output: 1x3x")
    (print oh)
    (print "x")
    (print ow)
    (set! mp (/ (static_arr_sum image (* (* (* 1 3) oh) ow)) (* (* (* 1 3) oh) ow)))
    (print "Mean pixel: ")
    (print mp)
    (set! _fp (fopen "/tmp/vae_output.bin" "wb"))
    (if (not (= _fp 0))
    (begin (fwrite image 8 (* (* (* 1 3) oh) ow) _fp) (fclose _fp) (print "Saved to /tmp/vae_output.bin")))
    (print "Done!")
    (exit_program 0)
  ))
