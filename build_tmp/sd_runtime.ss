...
(define (static_unet_sd15_forward x t context params n c h w)
  (begin
    ...
    "
    (set! emb_dim 1280)
    (set! emb (static_timestep_embedding_batch t emb_dim n 10000.0))
    (set! emb_mapped (make_float_array (* n emb_dim)))
    (dgemm_row_auto n emb_dim emb_dim 1.0 emb (+ params 0) 0.0 emb_mapped)
    (set! h_current (static_conv2d x (+ params 100) (+ params 101) n 4 320 h w 3 1 1))
    (set! h_skips (make_float_array (* (* (* (* 5 n) 1280) h) w)))
    (set! out (make_float_array (* (* (* n 4) h) w)))
    (static_arr_fill out 0.0 (* (* (* n 4) h) w))
    out
  ))
