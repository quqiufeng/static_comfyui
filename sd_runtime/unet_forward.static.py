...
def unet_forward(latent, timestep, context, data, n, hh, ww):
    ...
    while _i2 < _h:
        ...
        _i2 = _i2 + 1
    emb = _emb
    emb = linear_torch(emb, w_slice(data,1282707202,320*1280), w_slice(data,1282706562,320), n, 1280, 1280)
    arr_silu(emb, emb, n*1280)
    emb = linear_torch(emb, w_slice(data,1282912642,1280*1280), w_slice(data,1282912002,1280), n, 1280, 1280)

    h_cur = conv2d_torch(latent, w_slice(data,160,11520), w_slice(data,0,320), n, 4, 320, hh, ww, 3, 1, 1)
    _s[0] = h_cur
    ...
    h_cur = conv2d_torch(h_cur, w_slice(data,2261120,921600), w_slice(data,2260960,320), n, 320, 320, hh, ww, 3, 2, 1)
    hh = hh//2; ww = ww//2
    _s[3] = h_cur
    ...
    h_cur = conv2d_torch(h_cur, w_slice(data,30931520,3686400), w_slice(data,30931200,640), n, 640, 640, hh, ww, 3, 2, 1)
    hh = hh//2; ww = ww//2
    _s[6] = h_cur
    ...
    hh = hh*2; ww = ww*2
    h_cur = conv2d_torch(h_cur, w_slice(data,1216576002,14745600), w_slice(data,1216575362,1280), n, 1280, 1280, hh, ww, 3, 1, 1)
    ...
    hh = hh*2; ww = ww*2
    h_cur = conv2d_torch(h_cur, w_slice(data,1275277442,3686400), w_slice(data,1275277122,640), n, 640, 640, hh, ww, 3, 1, 1)
    ...
    h_cur = conv2d_torch(h_cur, w_slice(data,620658400,320), w_slice(data,620658240,320), n, 320, 4, hh, ww, 3, 1, 1)
    return h_cur
