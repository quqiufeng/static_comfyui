...
(define torch_std_cuda_synchronize
  (foreign-procedure "torch_std_cuda_synchronize" () void))
(define torch_std_euler_step
...
(define torch_std_sdxl_get_pooled
...
(define (static_sample_euler unet_fn x sigmas text_emb_cond text_emb_uncond cfg_scale)
  (begin
    ...
    (do ((i 0 (+ i 1))) ((>= i n_steps))
      (set! sigma_t (torch_std_narrow sigmas 0 i 1))
      (set! sigma_next (torch_std_narrow sigmas 0 (+ i 1) 1))
      (set! cond_eps (unet_fn x sigma_t text_emb_cond))
      (set! uncond_eps (unet_fn x sigma_t text_emb_uncond))
      (set! x (torch_std_euler_step x sigma_t sigma_next cond_eps uncond_eps cfg_scale))
      (torch_std_cuda_synchronize))
    x
  ))
...
(define (SD15Pipeline_txt2img self prompt steps cfg sampler_name scheduler_name height width seed denoise sigma_min sigma_max)
  (letrec ((wrapped_fn (lambda (x sigma te)
    (SD15Pipeline_unet_fn x sigma te))))
    (begin
...
(define (SDXLPipeline_txt2img self prompt steps cfg sampler_name scheduler_name height width seed denoise sigma_min sigma_max)
  (letrec ((wrapped_fn (lambda (x sigma te)
    (if (or (equal? te text_emb) (equal? te text_emb_cond))
        (SDXLPipeline_unet_fn x sigma te pooled_emb)
        (SDXLPipeline_unet_fn x sigma te pooled_emb_uncond)))))
    (begin
      (set! text_emb (SDXLPipeline_encode_prompt self prompt))
      (set! pooled_emb (torch_std_sdxl_get_pooled))
      (set! text_emb_cond text_emb)
      (set! text_emb_uncond (SDXLPipeline_encode_prompt self ""))
      (set! pooled_emb_uncond (torch_std_sdxl_get_pooled))
